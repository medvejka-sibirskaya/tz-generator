...
POLISH_PROMPT = (
    ...
    "2. В готовом ТЗ не должно оставаться пустых разделов, знаков-заглушек («—») "
    "и пропущенных пунктов — заполни каждый правдоподобным содержанием по смыслу "
    "проекта: сформулируй, что в таких разделах обычно указывают. "
    "Если ты дополнял(а) хотя бы один раздел, добавь отдельную строку ЗАГЛАВНЫМИ БУКВАМИ "
    "«ВНИМАНИЕ: ЧАСТЬ РАЗДЕЛОВ ДОПОЛНЕНА АВТОМАТИЧЕСКИ — ПРОВЕРЬТЕ ИХ ПЕРЕД СОХРАНЕНИЕМ» "
    "дважды: в самое начало документа (сразу после заголовка) и в самый конец "
    "(перед подписью о генерации). "
    "Внутри текста никаких приписок и пометок не оставляй. "
...
def ai_polish(text: str, model_name: str = "") -> tuple[str, bool]:
    ...
    try:
        ...
        # Стабильные ветки — с /latest (yandexgpt, yandexgpt-lite), версионные
        # слаги (yandexgpt-5.1) — без него, иначе 404.
        slug = base_model if base_model.endswith("/latest") or base_model[-1].isdigit() else base_model + "/latest"
        model = base_model if base_model.startswith("gpt://") else f"gpt://{LLM_FOLDER_ID}/{slug}"
